	...
	def checkAllow(self):
		with open("/etc/bind/named.conf.options", 'r') as f:
			...
			if "allow-recursion" not in conf:
				log.error("Use allow-recursion to restric recursive queries to trusted clients.")
			if "allow-query" not in conf:
				log.error("Use allow-query to restric queries to trusted clients.")
			if "allow-transfer" not in conf:
				log.error("Use allow-transfer to restirct zone transfer to trusted hosts.")
