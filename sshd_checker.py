...
def check_root():
    if sshd["permitrootlogin"] in ("yes", "prohibit-password", "without-password"):
        print("You should not use root account.", "Set PermitRootLogin to no please.", sep=' ', file=stderr)

def check_port():
    if sshd['port'] == '22':
        print("Port number should not be the default (22)", "Set Port to other please. For example: {}".format(randint(5000, 65535), sep=' ', file=stderr))

def check_logingracetime():
    if int(sshd["logingracetime"]) > 25:
        print("LoginGraceTime is very high, if you do not need so much, reduce the value of this variable.", file=stderr)

def check_passauthentication():
    if sshd["passwordauthentication"] == 'yes' or sshd["challengeresponseauthentication"] == 'yes':
        print("You should disable keyboard-interactive and use ssh keys instead (or convine it, for example ssh keys + OTP codes). Make sure than PasswordAuthentication and ChallengeResponseAuthentication is both disabled.", file=stderr)

def check_2fa():
    with open("/etc/pam.d/sshd", 'r') as f:
        sshd_pam = f.read()
    if re.match("\s*auth\s*required\s*pam_google_authenticator.so*", sshd_pam):
        print("It is recommended use 2FA.", file=stderr)
        return 0
...
def login_filter():
    if (not "allowusers" in sshd.getOptions()) or (not "allowgroups" in sshd.getOptions()):
        print("Filter users/groups with AllowUSers, AllowGroups.", file=stderr)

def check_privilege_separation():
    if sshd["useprivilegeseparation"] != "sandbox":
        print("Please, use \"UsePrivilegeSeperation sandbox\" option.", file=stderr)

def check_subsystem():
    if "subsystem" in sshd.getOptions():
        print("If you not really need {} disable it.".format(sshd["subsystem"]), file=stderr)
